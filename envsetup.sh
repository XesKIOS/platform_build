...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${XESKI_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    XESKI_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/xeski/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.xeski\.version/s/.*=//p' $OUT/system/build.prop`
        ZIPFILE=xeski-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
